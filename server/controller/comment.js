...
class commentController {
    ...
    async getCommentOfPost(req, res, next) {
        ...
        console.log(postID);
        const allPost = await Comment.find({ postID: postID }).populate(
            "userID"
        );
    ...
    async deleteComment(req, res, next) {
        ...
        if (userID === comment.userID) {
            ...
            if (
                postOfComment &&
                postOfComment._id.toString() === comment.postID.toString()
            ) {
                await comment.deleteOne();

                await postOfComment.update({
                ...
                return res
                    .status(200)
                    .json(
                        "The post doesn't exist or your comment not in this post !"
                    );
            }
        } else {
